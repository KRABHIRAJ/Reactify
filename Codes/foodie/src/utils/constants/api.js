export const baseUrl = 'https://swiggy-clone-69.krabhiraj99.workers.dev';
...
export const getAPIUrl = (lat, lng) => {
    return `${baseUrl}/dapi/restaurants/list/v5?lat=${lat}&lng=${lng}&is-seo-homepage-enabled=true&page_type=DESKTOP_WEB_LISTING`
}
...
export const getCarouselAPIUrl = (lat, lng, collId, tags) => {
    return `${baseUrl}/dapi/restaurants/list/v5?lat=${lat}&lng=${lng}&collection=${collId}&tags=${tags}&sortBy=&filters=&type=rcv2&offset=0&page_type=null`
}

export const getRestaurantMenuUrl = (lat, lng, restaurantId) => {
    return `${baseUrl}/dapi/menu/pl?page-type=REGULAR_MENU&complete-menu=true&lat=${lat}&lng=${lng}&restaurantId=${restaurantId}&catalog_qa=undefined&submitAction=ENTER`
}
